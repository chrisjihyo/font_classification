...
for font in font_list :
    font_path = f'C:/Users/USER/Desktop/동아리/글꼴 프로젝트/논문자료/실험/결과/OpenCV_EasyOCR/sentence_test_result_{font}'
    os.mkdir(font_path)
    

    for num in file_range : 
        os.mkdir(f"{font_path}/sentence_test_result_{font}_{num}")
        os.mkdir(f"{font_path}/sentence_test_result_{font}_{num}/sentence_test_result_{font}_{num}_원본")
        ...
        large = np.array(img)
        gray = cv2.cvtColor(large, cv2.COLOR_BGR2GRAY) #BGR 형식에서 그레이스케일로 변환하는 부분
        ...
        for N in range(len(image_cut_list)) :
            image_path = f'{font_path}/sentence_test_result_{font}_{num}/sentence_test_result_{font}_{num}_원본/sentence_test_result_{font}_{num}_원본_{N}.jpg'
            image_cut_list[N].save(image_path,'jpeg')
            #일단 나왔던 것들 모두 jpeg 형태로 원본에 저장하고 다시 가지고 와서 EasyOCR로 열기
            
            image = Image.open(image_path).convert('L')
            results = reader.readtext(np.array(image))

            for detection in results: #results에 있는 것들은 되도록이면 한 글자이겠지만, 혹시 OpenCV로도 완전히 구분하지 못한 것들이 있을 수 있음
                
                #만약 인식된 문자가 한글이면 정상으로, 아니라고 판단하면 비정상으로 분류
                if contains_korean(detection[1]) == True :
                    image_cut_list[N].save(f'{font_path}/sentence_test_result_{font}_{num}/sentence_test_result_{font}_{num}_정상/sentence_test_result_{font}_{num}_정상_{N}.png')
                    continue